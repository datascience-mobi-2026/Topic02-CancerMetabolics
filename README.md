...
Tutor: [Duc Thien Bui](mailto:duc-thien.bui@bioquant.uni-heidelberg.de)
...
The goal of this project is to perform a **pan-cancer analysis of these cancer hallmarks and metabolic pathways**, using datasets of genes expression and proteome data from a large collection of tumor samples. We will use the TCGA (The Cancer Genome Atlas) collection of RNA-seq and proteome data, across a large number of tumor types and patients (hence *pan-cancer*) and identify the activity of these hallmarks by using a collection of gene sets which describe the activity of these pathways. Then, each group in this project will focus on a specific cancer type and compare these hallmarks between the cancer samples and matched normal control.
...
(5) **Analyse proteome data**: for each cancer type, you also have access to proteome data for a selection of tumor patients. Compare the proteome data with the gene expression data for the genes available in both datasets (correlation?). Try to determine the pathway activities using the proteome data (Beware: there are much fewer proteins available than genes! Image a workaround for this by using e.g. only the available genes from a pathway.)

(6) **Predict pathway activities using a  regression model**: predict the activity of a metabolic pathway using the other hallmarks/metabolic pathways as explanatory variables
...
tcga_tumor_norm = readRDS("path/to/your/directory/tcga_tumor_normal_datascience_proj_2026.RDS")
...
[1] 9741   37
```
The third object is a list with 5 entries corresponding to 5 tumor types (BRCA = breast cancer; KIRC = kidney renal carcinoma; LUAD = lung adenocarcinoma; PRAD = prostate adenocarcinoma; THCA = Thyroid carcinoma); each entry of the list is itself a list with 4 entries, corresponding to (1) a data frame with expression values for tumor samples, (2) a data frame with expression values for normal samples from the same patients, (3) a data frame with clinical annotations for these patients, (4) a dataframe containing proteome information.
